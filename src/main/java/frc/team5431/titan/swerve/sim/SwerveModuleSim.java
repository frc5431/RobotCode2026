...
public class SwerveModuleSim {
  ...
  Rotation2d cursteerAngle =
      Rotation2d.fromDegrees(0); // 0 = toward front, 90 = toward left, 180 = toward back, 270
  // = toward right
  ...
  public SwerveModuleSim(
    ...
    this.driveMotor =
        new MotorGearboxWheelSim(driveMotor, wheelGearRatio, wheelRadius_m, wheelGearboxLossFactor);
  ...
  void update(double dtSeconds) {
    ...
    // Assume idealized azimuth control - no "twist" force at contact patch from
    // friction or robot motion.
  ...
  ForceAtPose2d getWheelMotiveForce() {
    return new ForceAtPose2d(
        new Force2d(driveMotor.getGroundForce_N(), cursteerAngle), curModulePose);
  }
