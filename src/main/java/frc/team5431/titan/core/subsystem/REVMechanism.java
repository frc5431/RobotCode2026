...
public abstract class REVMechanism implements Subsystem {
   ...
   * and Max Motion positional or velocity control, and WPILib Units
   *
  ...
  public boolean getVelocitySetpointGoal(AngularVelocity target, AngularVelocity error) {
    if (attached) {
      if (Calc.approxEquals(
          motor.getEncoder().getVelocity(), target.in(Units.RPM), error.in(Units.RPM))) {
        return true;
      }
    }
    return false;
   ...
   * @return true if the motor's angle position is within the error of the target angle position
   */
  ...
  /**
   * Checks if the motor is reaching the rotational setpoint Uses absolute encoder
   *
   * @param target the target rotation angle
  ...
  public void setMotorPosition(Angle position) {
    if (attached) {
      motor
          ...
          .setReference(position.in(Units.Rotations), ControlType.kPosition, ClosedLoopSlot.kSlot0);
    }
  }
   ...
   *
   * @param position Measure of Angle; Converted to Rotations
   * @param arbff A value from which is represented in voltage applied to the motor after the result
   *     of the specified control mode. The units for the parameter is Volts.
   */
  public void setMotorPosition(Angle position, double arbff) {
    if (attached) {
      motor
          .getClosedLoopController()
          .setReference(
              position.in(Units.Rotations), ControlType.kPosition, ClosedLoopSlot.kSlot0, arbff);
    }
  }
   ...
   *
   * @param velocity Mesure of Velocity in Terms of Angle; Converted to Rotations Per Minute
   */
  public void setMMVelocity(AngularVelocity velocity) {
    if (attached) {
      motor
          .getClosedLoopController()
          .setReference(
              velocity.in(Units.RPM), ControlType.kMAXMotionVelocityControl, ClosedLoopSlot.kSlot0);
    }
  }
  ...
  public void setVelocity(AngularVelocity velocity) {
    if (attached) {
      motor
          .getClosedLoopController()
          .setReference(velocity.in(Units.RPM), ControlType.kVelocity, ClosedLoopSlot.kSlot0);
    }
  }
  ...
  public void setMMPosition(Angle position) {
    if (attached) {
      motor
          .getClosedLoopController()
          .setReference(position.in(Units.Rotations), ControlType.kMAXMotionPositionControl);
    }
  }
  ...
  public void setMMPosition(DoubleSupplier position) {
    if (attached) {
      motor
          .getClosedLoopController()
          .setReference(position.getAsDouble(), ControlType.kMAXMotionPositionControl);
    }
  }
  ...
  public void setMMPosition(Angle position, ClosedLoopSlot slot) {
    if (attached) {
      motor
          .getClosedLoopController()
          .setReference(position.in(Units.Rotations), ControlType.kMAXMotionPositionControl, slot);
    }
  }
  ...
  public static class Config {
    ...
    public void configPositionWrapping(boolean enabled, Angle max, Angle min) {
      sparkConfig.closedLoop.positionWrappingEnabled(enabled);
      sparkConfig.closedLoop.positionWrappingInputRange(
          min.in(Units.Rotation), max.in(Units.Rotation));
    }
    ...
    public void configSmartCurrentLimit(Current stallLimit, Current supplyLimit) {
      sparkConfig.smartCurrentLimit(
          (int) stallLimit.in(Units.Amps), (int) supplyLimit.in(Units.Amps));
    }
     ...
     * @param velocity desired crusing velocity of motor (setpoint) (Rotations Per Minute)
     */
    ...
    /**
     * @param acceleration desired ROC of motor (Rotations Per Minute)
     * @param velocity desired crusing velocity of motor (Rotations Per Minute)
     * @param error allowed error in RPM
     */
    public void configMaxMotion(
        AngularVelocity velocity, AngularVelocity acceleration, AngularVelocity error) {
      sparkConfig.closedLoop.maxMotion.maxAcceleration(acceleration.in(Units.RPM));
      sparkConfig.closedLoop.maxMotion.maxVelocity(velocity.in(Units.RPM));
     ...
     *
     * @param gearRatio Ratio of Relvative encoder rotations relative to mechanism's rotary output
     */
