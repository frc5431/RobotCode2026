...
public class KinematicsSolver {
  ...
  private double solveSegment2Angle(Translation2d goal) {
    double p1 =
        Math.pow(goal.getX(), 2) + Math.pow(goal.getY(), 2) - Math.pow(l1, 2) - Math.pow(l2, 2);
    double p2 = 2 * l1 * l2;
  ...
  /**
   * This converts a goal to two angles for an arm. Essentially meaning we can say, "arm, go there."
   * instead of saying, "Arm set angle to this and this."
   *
   * @param goal The position at the end of the arm
   ...
   * instead of saying, "Arm set angle to this and this."
   *
   * <p>(Warning: Contains side effect of setting default to the one used here)
   *
   * @param goal The position at the end of the arm
