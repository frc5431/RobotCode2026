...
public class Blinkin extends SubsystemBase {
  ...
  public Command ledCommand(COMMAND cmd) {
    if (cmd == COMMAND.PREV) return Commands.runOnce(() -> this.prevPattern());
    else return Commands.runOnce(() -> this.nextPattern());
  }
  ...
  public Command ledRunCommand(COMMAND cmd) {
    if (cmd == COMMAND.PREV) return Commands.run(() -> this.prevPattern(), this);
    else return Commands.run(() -> this.nextPattern(), this);
  }
}
